...
% add constraints
for k=2:length(Mod)
  U.p = RES.p{k}.*(RES.r{1}>0.02);
  U.r = RES.r{k}.*(RES.r{1}>0.02);
  Contrast = setfield(Contrast,Mod{k},U);
end

% put back into roiTs like structure for display
roiTs = ck_insert_anatomy(roiTs,'D04PE1');
...
% Avoc, cage, mvoc, nsnd, rttl
R = insert_contrast(roiTs,Contrast,'stim1',RoiNum);
% cla_dsproits(R,'audit',0.06,[2,6],'r')
...
save('Dataset_natsounds.mat','roiTs','Contrast','RoiNum','Model','BigAna','Allmaps');
